...
export const AuthProvider = ({ children }) => {
    ...
    const checkAuth = async () => {
      ...
      if (savedToken) {
        try {
          ...
          localStorage.removeItem('token');
          localStorage.removeItem('roamiq_active_conv_id');
          setToken(null);
          setUser(null);
        }
      } else {
  ...
  const login = async (credentials) => {
    try {
      // Clear AI session to ensure a clean slate for the new user
      localStorage.removeItem('roamiq_active_conv_id');
  ...
  const logout = () => {
    localStorage.removeItem('token');
    localStorage.removeItem('roamiq_active_conv_id'); // Reset AI session
    setUser(null);
